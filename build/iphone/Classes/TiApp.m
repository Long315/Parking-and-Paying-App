...
- (void)boot
{
  DebugLog(@"[INFO] %@/%@ (%s.5ef0c56)", TI_APPLICATION_NAME, TI_APPLICATION_VERSION, TI_VERSION_STR);
...
- (NSString *)systemUserAgent
{
  ...
  NSString *kMyNewAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s", "Appc", "eler", "ator", "Tita", "nium"];
  return [NSString stringWithFormat:@"%@/%s (%@)", kMyNewAppUserAgentPrefix, TI_VERSION_STR, currentDeviceInfo];
}
...
+ (NSDictionary *)tiAppProperties
{
  ...
  if (props == nil) {
    ...
    if (jsonData) {
      ...
      // If we have no data...
      // This should never happen on a MyNewApp app using the node.js CLI
